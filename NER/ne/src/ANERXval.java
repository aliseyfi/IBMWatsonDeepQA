...
public class ANERXval {
	...
	static File outputEvaFile = null;
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException {
		...
		outputEvaFile = new File(args[0], "outputEva.txt");
		if (outputEvaFile.exists() == false) {
			outputEvaFile.createNewFile();
		}

		BufferedWriter writer = new BufferedWriter(new FileWriter(outputEvaFile));
		...
		File corpusFile = new File(anerDir, "output_data");
		...
		report(writer, "    NE Corpus", corpusFile);
		report(writer, "    Location Gazetteer", locGazFile);
		report(writer, "    Organization Gazetteer", orgGazFile);
		report(writer, "    Person Gazetteer", persGazFile);

		System.out.println("\nParameters");
		writer.write("\nParameters \n");
		System.out.println("   N-Gram=" + HMM_N_GRAM_LENGTH);
		writer.write("   N-Gram=" + HMM_N_GRAM_LENGTH + "\n");
		System.out.println("   Num chars=" + NUM_CHARS);
		writer.write("   N-Gram=" + HMM_N_GRAM_LENGTH + "\n");
		System.out.println("   Interpolation Ratio=" + HMM_INTERPOLATION_RATIO);
		writer.write("   Interpolation Ratio=" + HMM_INTERPOLATION_RATIO + "\n");
		System.out.println("   Number of Analyses Rescored=" + NUM_ANALYSES_RESCORED);
		writer.write("   Number of Analyses Rescored=" + NUM_ANALYSES_RESCORED + "\n");
		System.out.println("   Including MISC entity type=" + INCLUDE_MISC);
		writer.write("   Including MISC entity type=" + INCLUDE_MISC + "\n");
		System.out.println("   Use dictionary=" + USE_DICTIONARY);
		writer.write("   Use dictionary=" + USE_DICTIONARY + "\n\n");
		...
		writer.write("    Location Dict Entries=" + locDict.length + "\n");
		System.out.println("    Organization Dict Entries=" + orgDict.length);
		writer.write("    Organization Dict Entries=" + orgDict.length + "\n");
		System.out.println("    Person Dict Entries=" + persDict.length);
		writer.write("    Person Dict Entries=" + persDict.length + "\n");

		List<Chunking> sentences = parseANER(writer, corpusFile);
		...
		ChunkerEvaluator evaluator = new ChunkerEvaluator(null); // set chunker
																	// in loop
		...
		for (int fold = NUM_FOLDS; --fold >= 0;) {
			...
			writer.write("FOLD= " + fold + "start sent= " + startTestFold + "end sent= " + endTestFold + "\n");
			...
			printEval(writer, "FOLD=" + fold, foldEvaluator);
		}
		System.out.println("\n===============================================");
		writer.write("\n===============================================" + "\n");
		System.out.println("COMBINED CROSS-VALIDATION RESULTS");
		writer.write("COMBINED CROSS-VALIDATION RESULTS" + "\n");
		printEval(writer, "X-Val", evaluator);
		writer.flush();
		writer.close();
	}

	static void printEval(BufferedWriter writer, String msg, ChunkerEvaluator evaluator) throws IOException {
		...
		for (String tag : REPORT_TYPES) {
			...
			writer.write(msg + tag + "P =" + prEvalByType.precision() + "R=" + prEvalByType.recall() + "F="
		...
		System.out.printf("%10s  COMBINED P=%5.3f R=%5.3f F=%5.3f\n", msg, prEval.precision(), prEval.recall(),
				prEval.fMeasure());
		writer.write(
				msg + "COMBINED P =" + prEval.precision() + "R=" + prEval.recall() + "F=" + prEval.fMeasure() + "\n");
	...
	static List<Chunking> parseANER(BufferedWriter writer, File corpusFile) throws IOException {
	...
	static Chunking toChunking(List<String> tokens, List<String> tags) {
		...
		// // System.out.println("tagging size : " + tagging.size());
		//
		// for (int n = 1; n < tagging.size(); n++) {
		// String tag = tagging.tag(n);
		// if (tagging.tag(n - 1).equals("B-LOC") &&
		// tagging.tag(n).equals("I-ORG")) {
		// System.out.println("test111111 " + index);
		// }
		//
		// if (tagging.tag(n - 1).equals("B-PERS") &&
		// tagging.tag(n).equals("I-ORG")) {
		// System.out.println("test222222 " + index);
		// }
		//
		// index++;
		// // System.out.println((n -1) +" : "+tag);
		// }
	...
	static void report(BufferedWriter writer, String msg, File file) throws IOException {
		System.out.println(msg + ": " + file.getCanonicalPath());
		writer.write(msg + ": " + file.getCanonicalPath());
		writer.newLine();
	}
